...
class GameServer(
    ...
    /** Returns a list of bot results (for observers and controllers) ordered on the score ranks on tick */
    private fun getResultsForObserversOnTick(): List<ResultsForObserver> {

        val results = mutableListOf<ResultsForObserver>()

        val scores = ResultsView.getResults(modelUpdater!!.getScoresOnTick(), participantMap.values)
        scores.forEach { score ->
            participantMap[score.participantId.botId]?.let { participant ->
